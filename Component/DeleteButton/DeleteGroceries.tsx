import {
  ...
  import { deleteElectronic, deleteGroce } from "../../requestApi";
  
  import groceDelete from "./DeleteButton.module.css";
  
  const DeleteGroceries = ({ product,handleReload}): JSX.Element => {
  ...
  console.log('hasDelete',hasDelete);
   ...
   const deleteProductApi=async ()=>{
    ...
    const response= await deleteGroce(product.groceId);
  ...
  const errorResponse=handleError(response)
    ...
    return (
      <>
        ...
        <Box className={isDelete ? groceDelete.ModalX : ""}>
          {isDelete ? (
