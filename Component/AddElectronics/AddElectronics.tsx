...
import { createElecroProduct } from "../../requestApi";
import FileBase from "react-file-base64";
const AddElectronics = ({ isOpen, handleIsOpen, handleReload }) => {
  const [name, setName] = useState<string>("");
  const [electId, setId] = useState<string>("");
  ...
  const sendProduct = async (e) => {
    ...
    const response = await createElecroProduct({
      electId,
      name,
      description,
      brand,
...
console.log(response.data.data,'resp cre');


    if (response) {
      console.log(isOpen,'iso inside');
