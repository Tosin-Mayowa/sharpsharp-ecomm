...
try {
  ...
  const collection = db.collection("wears");
...
return  res.status(200).json({message:'found all wears products' ,data:product});
} finally {
  ...
  if (req.method === "POST") {
    let mongoClient;
    const wearsProduct = req.body;
    try {
      mongoClient = await connectToCluster(connectionString);
      const db = mongoClient.db("SharpStore");
      const collection = db.collection("wears");
    await collection.insertOne(wearsProduct);

      res.status(200).json({ data: wearsProduct});
    } finally {
